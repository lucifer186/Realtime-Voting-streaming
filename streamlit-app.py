...
def update_data():
    ...
    if not data:  # If no data is received
        st.warning("No voting data available yet")
        return

    results = pd.DataFrame(data)
    
    # Check if required columns exist
    required_columns = ['candidate_id', 'candidate_name', 'party_affiliation', 'total_votes']
    if not all(col in results.columns for col in required_columns):
        st.warning("Incomplete voting data received")
        return
    ...
    if location_data:  # Only process if data exists
    ...
    st.session_state['last_update'] = time.time()# Sidebar layout
def sidebar():
